...
Proxy routing is OFF by default. Enable it by setting:

    SCRAPER_USE_PROXY=true   (or 1 / yes)

Usage in scrapers:
    from .proxy_manager import get_session

    session = get_session()          # plain Session when proxy is off; proxy Session when on
