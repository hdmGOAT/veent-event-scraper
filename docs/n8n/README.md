...
| [n8n-leads-sheets-sync.md](n8n-leads-sheets-sync.md) | Async **upsert** of `/api/events/?upcoming=1` (by `db_id`) into the `Events Sync` Google Sheets tab — upcoming events only, formats `event_date`, never deletes rows, preserves human/CRM columns; no scraping |
